...
def update_dashboard_config():
    ...
    try:
        ...
        if 'raspberry_pi_host' in data:
            ...
            RASPBERRY_PI_CONFIG['host'] = data['raspberry_pi_host']
            os.environ['RASPBERRY_PI_HOST'] = data['raspberry_pi_host']
            
        if 'raspberry_pi_port' in data:
            config.RASPBERRY_PI_PORT = int(data['raspberry_pi_port'])
            RASPBERRY_PI_CONFIG['port'] = int(data['raspberry_pi_port'])
            os.environ['RASPBERRY_PI_PORT'] = str(data['raspberry_pi_port'])
            
        logger.info(f"Dashboard 配置已更新: {config.RASPBERRY_PI_HOST}:{config.RASPBERRY_PI_PORT}")
        ...
        logger.error(f"更新 Dashboard 配置失敗: {e}")
        return jsonify({
            'success': False,
            'message': f'更新配置失敗: {str(e)}'
        })
