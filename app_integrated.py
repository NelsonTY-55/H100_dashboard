...
# 設備設定路由 - 重定向到 Dashboard 服務
@app.route('/db-setting')
def db_setting():
    """設備設定頁面 - 重定向到 Dashboard 服務"""
    logging.info(f'重定向設備設定頁面到 Dashboard 服務, remote_addr={request.remote_addr}')
    return redirect('http://localhost:5001/db-setting')

@app.route('/api/device-settings', methods=['GET', 'POST'])
def api_device_settings():
    """API: 重定向到 Dashboard 服務"""
    return redirect(f'http://localhost:5001/api/device-settings', code=307)

@app.route('/api/multi-device-settings')
def api_multi_device_settings():
    """API: 重定向到 Dashboard 服務"""
    return redirect('http://localhost:5001/api/multi-device-settings')

# Dashboard 相關路由 - 重定向到 Dashboard 服務
@app.route('/dashboard')
def flask_dashboard():
    """Dashboard 主頁面 - 重定向到 Dashboard 服務"""
...
def dashboard_stats():
    """API: 重定向到 Dashboard 服務"""
    return redirect('http://localhost:5001/api/dashboard/stats')

@app.route('/api/dashboard/device-settings')
def dashboard_device_settings():
    """API: 重定向到 Dashboard 服務"""
    return redirect('http://localhost:5001/api/dashboard/device-settings')

@app.route('/api/dashboard/chart-data')
def dashboard_chart_data():
    """API: 重定向到 Dashboard 服務"""
    return redirect('http://localhost:5001/api/dashboard/chart-data')

@app.route('/api/dashboard/devices')
def dashboard_devices():
    """API: 重定向到 Dashboard 服務"""
    return redirect('http://localhost:5001/api/dashboard/devices')
